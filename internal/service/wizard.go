...
`, username, secret))
	...
	transportType := prompt(reader, "Transport Type (http/https/redirect)", "https")
	
	hostsInput := prompt(reader, "SNI/Host List (comma separated)", "www.google.com,www.microsoft.com")
	hosts := strings.Split(hostsInput, ",")
	for i := range hosts {
		hosts[i] = strings.TrimSpace(hosts[i])
	}
	hostsStr := fmt.Sprintf("%q", hosts)
	hostsStr = strings.ReplaceAll(hostsStr, " ", ", ")

	// --- Generate config ---
	id := randomID()
	filename := fmt.Sprintf("config_%s.toml", id)

	config := fmt.Sprintf(`# Lucy Stealth Tunnel Server Configuration
# Generated by wizard
...
`, listen, hostname, certFile, keyFile, logLevel,
		usersBlock.String(), listen, caCertFile,
		transportType, hostsStr,
	...
	fmt.Printf("  1. Start server:  lucy-server run -c %s\n", filename)
	fmt.Printf("  2. Or register:   lucy-server service install %s\n", filename)
...
`)
	...
	hostsInput := prompt(reader, "SNI/Host List (comma separated)", "www.google.com")
	hosts := strings.Split(hostsInput, ",")
	for i := range hosts {
		hosts[i] = strings.TrimSpace(hosts[i])
	}
	...
	config := fmt.Sprintf(`# Lucy Stealth Tunnel Client Configuration
# Generated by wizard
...
`, server, username, secret, caCert, insecureStr,
	...
	fmt.Println()
	fmt.Println("Next steps:")
	fmt.Printf("  1. Start client:  lucy-client run -c %s\n", filename)
	fmt.Printf("  2. Or register:   lucy-client service install %s\n", filename)
